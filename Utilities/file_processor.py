...
class FileProcessor:
    ...
    def print_file_details(self):
        ...
        logging.info(
            f"MAC Times (Modified, Accessed, Created): {tuple(time.ctime(t) for t in self.mac_times)}"
        )
        ...
        if self.file_header:
            logging.info(
                f"Header (hex representation): {hexlify(self.file_header).decode()}"
            )
        else:
...
def main():
    while True:
        ...
        if isinstance(dir_path, str) and os.path.isdir(dir_path):
            ...
            logging.warning(
                f"Directory '{dir_path}' does not exist. Please enter a valid directory."
            )
