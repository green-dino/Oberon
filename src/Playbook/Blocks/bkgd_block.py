...
class BkgdBlock:
    def __init__(self, bkgnd_size: int, bkgnd_id: int, bitmap_id: int, flags: int, next_bkgnd_id: int, prev_bkgnd_id: int,
                 part_count: int, part_content_count: int, parts: List[Part], part_contents: List[PartContent],
                 bkgnd_name: str, bkgnd_script: str):
        self.bkgnd_size = bkgnd_size
        self.bkgnd_type = 'BKGD'
        self.bkgnd_id = bkgnd_id
        self.filler = 0
        self.bitmap_id = bitmap_id
        self.flags = flags
        self.next_bkgnd_id = next_bkgnd_id
        self.prev_bkgnd_id = prev_bkgnd_id
        self.part_count = part_count
        self.part_content_count = part_content_count
        self.parts = parts
        self.part_contents = part_contents
        self.bkgnd_name = bkgnd_name
        self.bkgnd_script = bkgnd_script

    def __repr__(self) -> str:
        return (f"BkgdBlock(bkgnd_size={self.bkgnd_size}, bkgnd_type='{self.bkgnd_type}', bkgnd_id={self.bkgnd_id}, "
                f"filler={self.filler}, bitmap_id={self.bitmap_id}, flags={self.flags}, next_bkgnd_id={self.next_bkgnd_id}, "
                f"prev_bkgnd_id={self.prev_bkgnd_id}, part_count={self.part_count}, part_content_count={self.part_content_count}, "
                f"parts={self.parts}, part_contents={self.part_contents}, bkgnd_name='{self.bkgnd_name}', bkgnd_script='{self.bkgnd_script}')")

def create_bkgd_block(bkgnd_size: int, bkgnd_id: int, bitmap_id: int, flags: int, next_bkgnd_id: int, prev_bkgnd_id: int,
                      parts: List[Part], part_contents: List[PartContent], bkgnd_name: str, bkgnd_script: str) -> BkgdBlock:
    part_count = len(parts)
    part_content_count = len(part_contents)
    return BkgdBlock(bkgnd_size, bkgnd_id, bitmap_id, flags, next_bkgnd_id, prev_bkgnd_id,
                     part_count, part_content_count, parts, part_contents, bkgnd_name, bkgnd_script)
...
if __name__ == "__main__":
    ...
    bkgnd_block = create_bkgd_block(2048, 1, 2, 0, 3, 4, parts, part_contents, "Background1", "Script1")
