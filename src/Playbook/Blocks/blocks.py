...
class Block:
    ...
    def __repr__(self) -> str:
        return (
            f"Block(block_size={self.block_size}, block_type='{self.block_type}', "
...
class StackBlock(Block):
    ...
    def __repr__(self) -> str:
        return (
            f"StackBlock(stack_size={self.block_size}, stack_type={self.stack_type}, stack_id={self.block_id}, "
...
def validate_blocks(blocks: List[Block]) -> bool:
    ...
    if not all(
        blocks[i].block_type == expected_first_blocks[i]
        for i in range(len(expected_first_blocks))
    ):
        return False
