    ...
    if isinstance(json_data, dict):
        filtered_data.extend(
            [
                ...
                if filter_key.lower() in str(key).lower()
                or filter_value.lower() in str(value).lower()
            ]
        )
...
def main():
    ...
    file_option = st.sidebar.selectbox(
        "Select how to load your JSON file:", ("Upload", "Local File")
    )
    ...
    if file_option == "Upload":
        ...
        if uploaded_file is not None:
            try:
                ...
                if search_term or filter_key or filter_value:
                    ...
                    filtered_data = filter_json_data(
                        json_content, filter_key, filter_value
                    )
                    if filtered_data:
        ...
        local_file_path = st.sidebar.text_input(
            "Enter path to your local JSON file:", ""
        )
