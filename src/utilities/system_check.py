...
from about_sys import SystemConfig

current_time, readable_time = SystemConfig.get_current_timestamp()
st.write("**Current timestamp:**", current_time)
st.write("**Readable time:**", readable_time)
# Display CPU count
cpu_count = SystemConfig.get_cpu_count()
st.write("**CPU Count:**", cpu_count)
...
st.write("**Total Memory:**", f"{total_mem} bytes")
st.write("**Available Memory:**", f"{avail_mem} bytes")
st.write("**Used Memory:**", f"{used_mem} bytes")
st.write("**Memory Usage:**", f"{mem_percent}%")

# Display disk information
disk_info = SystemConfig.get_disk_info()
for device, info in disk_info.items():
    st.write(f"**Disk {device}:**")
    st.write(f"  Total: {info['total']} bytes")
    st.write(f"  Used: {info['used']} bytes")
    st.write(f"  Free: {info['free']} bytes")
    st.write(f"  Usage: {info['percent']}%")
...
# Display Python version and platform information
python_version, platform, prefix = SystemConfig.get_python_info()
