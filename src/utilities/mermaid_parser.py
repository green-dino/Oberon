...
class MermaidParser:
    ...
    def prompt_user_nodes(self) -> Union[str, None]:
        ...
        """
        print(
            "Enter the nodes (type 'done' when finished, 'back' to go back, 'edit' to modify existing):"
        )
        while True:
            node = input("Node: ")
            if node.lower() == "done":
    ...
    def prompt_user_edges(self) -> Union[str, None]:
        ...
        """
        print(
            "Enter the edges in the format 'node1 --> node2' (type 'done' when finished, 'back' to go back, 'edit' to modify existing):"
        )
        while True:
            edge = input("Edge: ")
            if edge.lower() == "done":
                ...
                self.edges.append(
                    (edge.split("-->")[0].strip(), edge.split("-->")[1].strip())
                )
    ...
    def edit_nodes(self) -> None:
        ...
        while True:
            action = input(
                "Enter the number of the node to edit, 'delete <number>' to remove, or 'done' to finish: "
            )
                ...
                try:
                    ...
                    if 1 <= num <= len(self.nodes):
                        ...
                        if re.match(r"^\w+$", new_value):
                            ...
                            print(
                                "Invalid node name. Please use alphanumeric characters only."
                            )
                    else:
                        print("Invalid number.")
                except ValueError:
    ...
    def edit_edges(self) -> None:
        ...
        while True:
            action = input(
                "Enter the number of the edge to edit, 'delete <number>' to remove, or 'done' to finish: "
            )
            if action.lower() == "done":
                break
            elif action.startswith("delete"):
                ...
                try:
                    ...
                    if 1 <= num <= len(self.edges):
                        ...
                        if re.match(r"^\w+\s*-->\s*\w+$", new_value):
                            self.edges[num - 1] = (
                                new_value.split("-->")[0].strip(),
                                new_value.split("-->")[1].strip(),
                            )
                            print(f"Edge {num} updated to '{new_value}'.")
                        else:
                            print(
                                "Invalid edge format. Please use the format 'node1 --> node2'."
                            )
                    else:
                        print("Invalid number.")
                except ValueError:
