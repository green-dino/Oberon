...
from Utilities.file_operations import FileHandler
from Utilities.file_processor import FileProcessor
from Utilities.file_procssor import FileProcessor
...
for category, tasks in categories.items():
    for task in tasks:
        ...
        )  # Random count for demonstration
    ...
    with col:
        ...
        for method_name in methods:
            ...
            if st.button(
                button_label,
                key=button_key,
                on_click=lambda method=method_name: process.execute_mock_method(method),
            ):
...
df_time_series["Date"] = pd.date_range(
    start="1/1/2022", periods=len(df_time_series), freq="M"
)
